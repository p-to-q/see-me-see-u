...
2. **升档那一帧 —— 正常显示节拍下已证实，修法证伪（§10.8）**。固定 `seed=7` 的 3 场里，前两次升档
   **6/6** 命中 74–133ms；同三场 6 次忒修斯替换全都没有 ≥50ms LoAF，Q3 先收窄到整档升档。
   但“把未来材质 `compileAsync` 进后期 RT/MRT”不是可用修法：three r186 对对象根和整 scene 两种试法都报
   WebGPU fragment-output / color-target 不匹配，3 场 A/B 仍是 6/6 长帧；实现已完整撤回，不能把假预热留在运行时。
   下一步先做受控分段：分别关掉 `stage.pulse`、桶/材质变化和第一次真实 render，把 `creature.remorph` 的同步段与渲染段拆开量；
   在归因前不再猜 GPU，也不扩下一套缓存生命周期。
