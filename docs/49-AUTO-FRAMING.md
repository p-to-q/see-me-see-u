...
## 13 · 原始决定与这一帧真正采用的景别只有一个汇合点（2026-09-18）

分类器只能回答“这份摄像头姿态像全身还是上半身”；它不知道身体已经变成什么拓扑，也不知道第二具身体
是否真的通过预算画上台。旧接线让舞台、小屏和横纵跟随各自补一段条件，结果出现三种分裂：参数化
`rig` 只因 `drift = 1` 就被误判成非人形；预算只够一具身体时，小屏仍把 tracker 候选当作 companion；
工作台显示多人全景，却仍让主身体横向追人。

现在 `stage/effective-framing.ts` 是 app 编排层的唯一汇合点。它保留原始 `FramingDecision` 给腿部安全策略，
再用**当前有效形体**和**真正画上台的 companion**解析 `stageShot`：`rig` / `stub` / `towering` 即使在
完全漂移后仍可中景；非人形或未知形体从开始漂移起回全景；实际 companion 在场时整组全景。舞台景别、
横向快档、纵向跟随、小屏数字裁切与 HUD 共用这份结果。摄像头原生 `faceFraming` 仍只豁免“腿缺失”告警，
不会擅自把舞台推成中景。

小屏不再第二次用探测轨迹否决裁切；其余人的骨架透明度按 tracker 的选择顺序与当帧渲染预算计算，
调速器只留主身体时不会把未渲染的人画成“已有身体”。工作台的 companion 开关也会关闭横向跟随，
与正式链的多人让位一致。

确定性证据：42 条 app 定向测试通过；`?plan=rig&framing=upper` 的等价纯函数反证保留中景，四足漂移回全景，
多人 120 帧横向结果为 `yield` 且保持中线；九段 60Hz trace 的最大 16ms 跳变量继续守住既有限制
（景别进度 0.0300、速度变化 0.5212、裁切 0.0044、横向 0.0240、FOV 0.8913°、移轴 0.0018m）。
这些仍是合成证据；真人近 / 中 / 远、多人进出、调速器降级与真的原生 `faceFraming` 组合没有因此被宣称已验证。
